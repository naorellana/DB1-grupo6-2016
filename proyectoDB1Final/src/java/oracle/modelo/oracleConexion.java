...
public class oracleConexion {
    ...
    private String claveDB="PROYECTOB"; //usuario a la base de datos
    private String usuarioDB="PROYECTOB"; //usuario de la base de datos (schema de oracle)
    ...
    public Object [][] consultarDB1(String nombreTabla){
        ...
        return registro; // retorna un objeto con los datos y al llamarlo desde otra clase un bbjeto sera = oracleConexion.consultarDB1
    ...
    public Object [][] encabezadoTabla(String nombreTabla){
        ...
        oracleConexion conAux= new oracleConexion();
    ...
    public void actualizarDB(String comandoSQL){
        this.comandoSQL=comandoSQL;
        try{
            conectar(); // conectamos mediante la funion previa conectar
            estado.executeUpdate(comandoSQL) ;  //ejecutamos la sentencia o comando sql
            conexion.close(); //erramos la conexion
            if (estado.getUpdateCount() ==0){ /*verifica si la actualizacion es 0 y devuelve el mensaje que no actualizo la informacion */
                mensaje="No Se Realizo Ninguna Actualizacion Ingrese El Identificador Correcto " ; 
            }else{
                mensaje="Solicitud Procesada";
            }
        }
